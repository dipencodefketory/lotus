...
while i*request_quant <= len(products):
    r = afterbuy_api.get_shop_products({'ProductID': [product.internal_id for product in products[(i-1)*request_quant:i*request_quant]]})
    ...
    for p in p_tree:
        ...
        if product:
            try:
                ...
                db.session.commit()
                r = product.mp_update(marketplace.id, quantity=True, authorization=idealo_access)
                print(product.id)
                print(r.status_code)
