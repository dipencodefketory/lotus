...
from functions import str_to_float, money_to_float, marketplace_price_performance_measure
import afterbuy_api
import idealo_offer
...
r = afterbuy_api.get_sold_items({'DateFilter': ['AuctionEndDate', (datetime.now()-timedelta(days=3)-timedelta(hours=6)).strftime('%d.%m.%Y %H:%M:%S'), (datetime.now()-timedelta(days=3)).strftime('%d.%m.%Y %H:%M:%S')]})
...
for order in tree.findall('.//Order'):
    ...
    if check_sale is None:
        international = order.find('.//Country').text != 'D'
